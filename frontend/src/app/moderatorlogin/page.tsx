...
const ModeratorLoginPage = () => {
  ...
  const handleModeratorLogin = async () => {
    try {
      const response = await fetch('http://localhost:8082/api/moderatorlogin', {
        method: 'POST',
        headers: {
      ...
      if (response.ok && result.success) {
        ...
        alert(result.message || 'Moderator login failed. Please try again.');
      }
    } catch (error) {
      console.error('Error during moderator login:', error);
      alert('An error occurred during moderator login. Please try again.');
    }
  };
  ...
  return (
    <div style={{
      ...
      <h1 className="text-4xl font-bold text-black mb-10">Moderator Login</h1>
